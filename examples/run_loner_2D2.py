...
def run_trial(config, settings, settings_description = None, config_idx = None, trial_idx = None, dryrun: bool = False):
    ...
    # lidar_poses_init = torch.Tensor()
    lidar_poses = build_poses_from_df_2D(msgs[:,[8,9,10,11]],True)
    # lidar_poses_init = lidar_poses
    ...
    lidar_prev = msgs[0,[8,9,10,11]]
    start = 0
    end = 0
    while end<msgs.shape[0]-1:
        end = end+1
        if not np.sum(abs(msgs[end,[8,9,10,11]]-lidar_prev))<1:
            ...
            if start_lidar_pose is None:
                ...
                # print("start_lidar_pose",start_lidar_pose)
            gt_lidar_pose = start_lidar_pose.inverse() @ T_lidar
            # print("gt_lidar_pose",gt_lidar_pose)
            loner.process_lidar(lidar_scan, Pose(gt_lidar_pose))
            start = end
            lidar_prev = msgs[end,[8,9,10,11]]
            # if end-start<=3:
            #     start = end
            ...
            #         start_lidar_pose = T_lidar     
            #         # print("start_lidar_pose",start_lidar_pose)
            #     gt_lidar_pose = start_lidar_pose.inverse() 
            #     # print("gt_lidar_pose",gt_lidar_pose)
            #     loner.process_lidar(lidar_scan, Pose(gt_lidar_pose))
            #     start = end
            #     lidar_prev = msgs[end,[6,7,10,11]]
